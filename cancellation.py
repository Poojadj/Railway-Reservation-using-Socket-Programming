...
def cancel():
   ...
   try:
	...
	train_id = train_id[0]
	print(type(train_id))
	cur2.execute("""SELECT t.available_seat FROM train_status t where t.train_id = %s""",((train_id)))
	#print(hello)
        avail = cur2.fetchone()
	print(type(avail))
	avail = avail[0] + seats
	...
	cur.execute("""UPDATE train_status SET available_seat = %s, booked_seats = %s where train_id like %s""",(str(avail),str(booked),(train_id))	)
